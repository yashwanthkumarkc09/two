...
public class DemoOnTestCaseIfAuto extends  AllActions {
...
String actual;
String expected="Welcome to MeeHappyoij";
boolean check;
	...
	catch(Exception e)
	{
        ...
        try
        {
            actual=actions.GetObjectText(new DemoObjectsPage(driver).displayText);
        ...
        try
        {
            check=javamethod.StringEquals(actual,expected);
        ...
        {
            if(check){
actions.Click(new DemoObjectsPage(driver).happyMe);
}//If-End
else{
actions.Click(new DemoObjectsPage(driver).library);
}//Else-End
